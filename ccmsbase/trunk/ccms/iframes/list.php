...
<?php
	...
	*/
	if(isset($_GET['new']) && is_numeric($_GET['new'])){
		...
		* @see CCMSListServer::$tmp_newrowid for further information
		*/
		...
		*/
		if($_GET['subto'] == "base")
			...
			$dbid = $serverobject->get_dbid($_GET['subto']);

		$newid = $serverobject->add_new($_GET['lvl'], $dbid);
		...
		*/
		$serverobject->get_addjs($_GET['new'], $_GET['subto'], $_GET['lvl']);
...
<form method="POST" name="ccms_list_form" action="list.php?name=<?php echo $_GET['name'];?>" accept-charset="UTF-8">
	<?php
		if(!isset($_POST['rowid']))
			$_POST['rowid'] = false;

		if(!isset($_POST['startelement']))
			$_POST['startelement'] = false;
	?>
	<input type="text" value="<?php echo $_POST['rowid'];?>" name="rowid" />
	<input type="text" value="<?php echo $_POST['startelement'];?>" name="startelement" />
	<?php
	if(isset($_GET['new']))
		$_POST['startelement'] = false;
...
*/
 if(isset($_POST['rowid']) && $_POST['rowid'] !== false && is_numeric($_POST['rowid'])){ 
	...
	*/
	$serverobject->update_me($_POST, $serverobject->get_level($_POST['startelement']));
